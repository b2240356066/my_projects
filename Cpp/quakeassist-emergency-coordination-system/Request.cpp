...
int Request::computeEmergencyScore() const {
    ...
    if(type == "RESCUE" ){
        ...
        emergencyScore = emergencyLevel * 10 + numPeople * getRiskMultiplier();
    }

    else if(type == "SUPPLY" ){
        if(amount < 50){

             emergencyScore = emergencyLevel * 10 + amount; 
        }
        else{

            emergencyScore = emergencyLevel * 10 + 50;
        }
...
int Request::computeWorkloadContribution() const {
    ...
    if(type == "RESCUE" ){
        ...
        workload += numPeople * getRiskMultiplier();
    }
